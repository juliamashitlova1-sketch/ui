...
import type { DetectedState, DetectedTransition, GenerationOptions } from '@/lib/ai/types'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const transitions: DetectedTransition[] = (dbTransitions || []).map(t => ({
      id: t.id,
    ...
    const result = await generateCode({ states, transitions, effectsCount: 3 }, genOptions)
